...
CGA can be implemented based on different metrics corresponding to different definitions of the origin and the point at infinity. Different metrics results in different formulas for composition of CGA-vectors and in the formulas for determining lenghts and angles. In this API the origin is defined as $\epsilon_0=0.5(\epsilon_5-\epsilon_4)$ and the point at infinity as $\epsilon_\infty=\epsilon_4+\epsilon_5$ with $\epsilon_5^2=-1$. This corresponds to ganja.js, the book [Hildenbrand2009] and also the papers [Klepper2016].
